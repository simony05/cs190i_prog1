...
def convert_cellboxes(predictions, S=9):
    ...
    predictions = predictions.reshape(batch_size, S, S, 40)  # 4 boxes → 40 total values

    # Get bounding boxes and confidence scores for each of the 4 boxes
    bboxes = [
    ...
    # Find best bounding box (with max confidence)
    scores = torch.cat(confidences, dim=0)  # shape (4, B, S, S)
    best_box = scores.argmax(0).unsqueeze(-1)  # shape (B, S, S, 1)

    # Select best bounding box from bboxes list
    best_boxes = sum([
    ...
    # Get confidence of the best box (match index from best_box)
    best_confidence = sum([
